...
export function Newsletter({ title }: { title: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success('Inscription enregistrée. Bienvenue ! Votre adresse apparaît dans l’espace propriétaire de la démo.')
      setEmail('')
    } catch {
      toast.error('Inscription impossible, vérifiez l’adresse.')
    } finally {
      setSending(false)
    }
  }
    ...
    <section className="pb-20">
      <Reveal className="mx-auto max-w-6xl px-4">
        <div className="arch relative overflow-hidden bg-forest px-6 pb-12 pt-16 text-center text-cream sm:px-12">
          ...
          <h2 className="mx-auto mt-3 max-w-lg font-display text-4xl font-light leading-tight">{title}</h2>
          <form onSubmit={submit} className="mx-auto mt-7 flex max-w-md flex-col gap-2 sm:flex-row">
            ...
            <button disabled={sending} className="inline-flex items-center justify-center gap-2 rounded-full bg-terra px-6 py-3.5 text-sm font-medium text-cream hover:bg-nude hover:text-forest disabled:opacity-60"><Send size={15} /> Je m’inscris</button>
          </form>
