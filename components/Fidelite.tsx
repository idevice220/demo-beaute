...
export function Fidelite({ fidelity }: { fidelity: Site['fidelity'] }) {
  if (!fidelity.on) return null
  const stamped = 6
  const [head, ...rest] = fidelity.title.split(' est ')
  return (
    <section className="py-16">
      <div className="mx-auto grid max-w-6xl items-center gap-8 px-4 lg:grid-cols-[1fr_1fr]">
        <Reveal>
          ...
          <h2 className="mt-3 font-display text-4xl font-light leading-tight sm:text-5xl">{rest.length ? <>{head} est <em className="italic text-terra">{rest.join(' est ')}</em></> : fidelity.title}</h2>
          <p className="mt-4 max-w-md text-lg font-light text-muted">{fidelity.text}</p>
