          ...
          <Reveal>
            <div className="rounded-3xl bg-cream p-7 shadow-soft">
              ...
              <ul className="mt-5 space-y-2.5 text-[15px] text-ink/80">
                <li className="flex items-start gap-2"><MapPin size={16} className="mt-0.5 shrink-0 text-terra" /> {BUSINESS.address}</li>
                <li className="flex items-start gap-2"><Train size={16} className="mt-0.5 shrink-0 text-terra" /> RER E « Le Raincy – Villemomble », 3 min à pied</li>
                <li className="flex items-start gap-2"><Car size={16} className="mt-0.5 shrink-0 text-terra" /> Stationnement gratuit dans l’avenue</li>
              </ul>
              <div className="mt-6 flex flex-wrap gap-2">
                <a href={BUSINESS.maps} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-forest px-5 py-2.5 text-sm font-medium text-cream hover:bg-terra"><Navigation size={15} /> Itinéraire</a>
                <a href={BUSINESS.tel} className="inline-flex items-center gap-2 rounded-full border hairline px-5 py-2.5 text-sm font-medium hover:border-terra hover:text-terra"><Phone size={15} /> {BUSINESS.phone}</a>
              </div>
            </div>
          </Reveal>
          <Reveal delay={100}>
            <div className="relative h-full min-h-[360px] overflow-hidden rounded-3xl border hairline bg-sand-2 shadow-soft">
              <svg viewBox="0 0 600 420" className="absolute inset-0 h-full w-full" role="img" aria-label="Plan schématique : l’institut avenue de la Résistance, à 3 minutes de la gare RER E du Raincy">
                ...
                <text x="300" y="264" textAnchor="middle" fontSize="12" fill="#FBF8F3" fontWeight="600">L’Écrin · 12 av. de la Résistance</text>
                {/* parking */}
