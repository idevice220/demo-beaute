...
import type { CategoryT, RituelT, TeamT, TakenSlot } from '@/lib/content'
import type { Site } from '@/lib/settings'
import { bookableDays, slotsFor, dayLabel, dayLong, fmtH, isoDay, pad } from '@/lib/booking'
import type { BookingDetail } from './BookButton'

type Item = { id: string; name: string; min: number; price: number; cat: string; desc?: string }
const STEPS = ['Soin', 'Praticienne', 'Créneau', 'Coordonnées']
const NONE = 'Sans préférence'

function icsLink(day: Date, h: number, item: Item, siteName: string, address: string) {
  ...
  const body = ['BEGIN:VCALENDAR', 'VERSION:2.0', `PRODID:-//${siteName}//FR`, 'BEGIN:VEVENT', `UID:${start.getTime()}@lecrin`, `DTSTAMP:${f(new Date())}`, `DTSTART:${f(start)}`, `DTEND:${f(end)}`, `SUMMARY:${siteName} — ${item.name}`, `LOCATION:${address}`, 'END:VEVENT', 'END:VCALENDAR'].join('\r\n')
  return 'data:text/calendar;charset=utf-8,' + encodeURIComponent(body)
}

export function Booking({ categories, rituels, team, site, taken: initialTaken }: { categories: CategoryT[]; rituels: RituelT[]; team: TeamT[]; site: Site; taken: TakenSlot[] }) {
  const ALL = useMemo<Item[]>(() => [
    ...categories.flatMap((c) => c.soins.map((s) => ({ id: `s-${s.id}`, name: s.name, min: s.min, price: s.price, cat: c.name, desc: s.desc || undefined }))),
    ...rituels.map((r) => ({ id: `r-${r.id}`, name: r.name, min: r.min, price: r.price, cat: 'Rituels', desc: r.items.join(' · ') })),
  ], [categories, rituels])
  const TABS = useMemo(() => [...categories.map((c) => c.name), ...(rituels.length ? ['Rituels'] : [])], [categories, rituels])

  const [open, setOpen] = useState(false)
  const [step, setStep] = useState(0)
  const [cat, setCat] = useState(TABS[0] ?? '')
  ...
  const [taken, setTaken] = useState<TakenSlot[]>(initialTaken)
  ...
  useEffect(() => {
    const onOpen = (e: Event) => {
      ...
      reset()
      setDays(bookableDays(site.hours))
      if (d.soinId) {
    ...
    if (window.location.hash === '#reserver') { setDays(bookableDays(site.hours)); setOpen(true) }
    return () => window.removeEventListener('open-booking', onOpen)
  }, [reset, ALL, site.hours])
  ...
  /** Un créneau est pris si la praticienne choisie y est déjà réservée, ou si toute l'équipe l'est. */
  const isTaken = useCallback((date: string, time: string) => {
    const at = taken.filter((t) => t.date === date && t.time === time)
    if (staff !== NONE) return at.some((t) => t.staff === staff || t.staff === NONE && at.length >= team.length)
    return at.length >= Math.max(1, team.length)
  }, [taken, staff, team.length])
  const slots = useMemo(() => (day && item ? slotsFor(site.hours, day, item.min, (time) => isTaken(isoDay(day), time)) : []), [day, item, site.hours, isTaken])
  ...
  async function confirm() {
    if (!item || !day || slot === null) return
    setSending(true)
    const time = `${pad(Math.floor(slot))}:${pad(Math.round((slot % 1) * 60))}`
    try {
      const res = await fetch('/api/bookings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ item: item.name, staff, date: isoDay(day), time, duration: item.min, price: item.price, name: form.name, email: form.email, phone: form.phone, note: form.note }),
      })
      const data = await res.json().catch(() => ({}))
      if (res.status === 409) { toast.error(data.error || 'Créneau indisponible'); setTaken((t) => [...t, { date: isoDay(day), time, staff }]); setSlot(null); setStep(2); return }
      if (!res.ok) throw new Error()
      setTaken((t) => [...t, { date: isoDay(day), time, staff }])
      setDone(true)
      toast.success('Rendez-vous confirmé ! Il apparaît déjà dans l’agenda de l’espace propriétaire de la démo.')
    } catch {
      ...
      <div role="dialog" aria-modal="true" aria-label="Réserver un soin" onClick={(e) => e.stopPropagation()} className="flex max-h-[94vh] w-full max-w-3xl flex-col overflow-hidden rounded-t-3xl bg-cream shadow-lift sm:rounded-3xl">
          ...
          {done && item && day && slot !== null ? (
            <div className="flex flex-col items-center py-6 text-center">
              ...
              <div className="mt-6 w-full max-w-sm rounded-2xl border hairline bg-sand p-5 text-left">
                ...
                <p className="mt-3 font-display text-2xl">{item.price.toLocaleString('fr-FR')}&nbsp;€ <span className="text-sm text-muted">réglés sur place</span></p>
              </div>
              <a href={icsLink(day, slot, item, site.name, site.address)} download="rendez-vous.ics" className="mt-5 inline-flex items-center gap-2 rounded-full bg-forest px-5 py-3 text-sm font-medium text-cream hover:bg-terra"><CalendarPlus size={16} /> Ajouter à mon agenda</a>
                    ...
                    <li key={s.id}>
                      <button onClick={() => setItem(s)} className={`flex w-full items-center gap-4 px-2 py-3.5 text-left transition-colors ${on ? 'bg-terra-tint/60' : 'hover:bg-sand'}`}>
                        ...
                        <span className="shrink-0 text-right">
                          <span className="block font-display text-xl">{s.price.toLocaleString('fr-FR')}&nbsp;€</span>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              {team.map((t) => (
                <button key={t.id} onClick={() => setStaff(t.name)} className={`flex items-center gap-4 rounded-2xl border p-4 text-left ${staff === t.name ? 'border-forest bg-terra-tint/50' : 'hairline hover:border-terra'}`}>
            ...
            <>
              ...
              {day ? (
                <>
                  <p className="eyebrow mt-4 text-muted">{dayLong(day)} · {item?.min} min{staff !== NONE ? ` · avec ${staff}` : ''}</p>
            ...
            <div className="grid gap-6 sm:grid-cols-[1fr_0.8fr]">
              ...
              <div className="h-fit rounded-2xl bg-sand p-5">
                ...
                <p className="mt-3 border-t hairline pt-3 font-display text-3xl">{item?.price.toLocaleString('fr-FR')}&nbsp;€</p>
        ...
        {!done && (
          <div className="flex items-center justify-between gap-3 border-t hairline bg-cream px-5 py-4 sm:px-7">
            ...
            <div className="hidden text-sm text-muted sm:block">{item ? `${item.name} · ${item.min} min · ${item.price.toLocaleString('fr-FR')} €` : 'Choisissez un soin'}</div>
            {step < 3 ? (
