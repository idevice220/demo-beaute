...
import { AVIS, BUSINESS } from '@/lib/data'
    ...
    <section id="avis" className="py-20 lg:py-28">
      <div className="mx-auto max-w-6xl px-4">
        <div className="flex flex-wrap items-end justify-between gap-6">
          ...
          <div className="mb-12 flex items-center gap-4">
            <div className="text-right">
              <p className="font-display text-5xl leading-none">{BUSINESS.rating.toLocaleString('fr-FR')}</p>
              <p className="text-xs text-muted">{BUSINESS.reviews} avis Google</p>
            </div>
        ...
        <div ref={row} className="no-scrollbar snap-row flex gap-5 overflow-x-auto pb-2">
          {AVIS.map((a) => (
            <article key={a.name} className="snap-item flex w-[85%] shrink-0 flex-col rounded-3xl border hairline bg-cream p-7 sm:w-[48%] lg:w-[32%]">
              ...
              <div className="mt-6 flex items-center justify-between border-t hairline pt-4">
                ...
                <span className="flex gap-0.5">{Array.from({ length: 5 }).map((_, i) => <Star key={i} size={12} className="fill-gold text-gold" />)}</span>
              </div>
            </article>
          ))}
        </div>
