...
export function Galerie() {
  ...
  const step = useCallback((d: number) => setIdx((i) => (i === null ? null : (i + d + GALLERY.length) % GALLERY.length)), [])
  useEffect(() => {
    if (idx === null) return
    ...
    <section id="galerie" className="py-20 lg:py-28">
      <div className="mx-auto max-w-6xl px-4">
        <div className="flex flex-wrap items-end justify-between gap-6">
          ...
          <Reveal delay={100}>
            <a href={BUSINESS.instagram} className="mb-12 inline-flex items-center gap-2 rounded-full border hairline px-5 py-2.5 text-sm font-medium hover:border-terra hover:text-terra"><Instagram size={16} /> @lecrin.leraincy</a>
          </Reveal>
        </div>

        <div className="columns-2 gap-3 md:columns-3 md:gap-4">
          {GALLERY.map((g, i) => (
            <Reveal key={g.src} delay={(i % 3) * 60} className="mb-3 break-inside-avoid md:mb-4">
      ...
      {idx !== null && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-ink/90 p-4 backdrop-blur-sm" onClick={close} role="dialog" aria-modal="true">
          ...
          <figure className="w-full max-w-3xl" onClick={(e) => e.stopPropagation()}>
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-ink"><Image src={GALLERY[idx].src} alt={GALLERY[idx].alt} fill sizes="90vw" className="object-contain" /></div>
            <figcaption className="mt-3 flex justify-between text-sm text-cream/80"><span className="font-display text-lg">{GALLERY[idx].alt}</span><span>{idx + 1}/{GALLERY.length}</span></figcaption>
          </figure>
        </div>
      )}
    </section>
