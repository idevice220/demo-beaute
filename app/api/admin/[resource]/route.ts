...
export async function GET(_: Request, { params }: { params: { resource: string } }) {
  ...
  const rows = await table(def.model).findMany({ orderBy: def.orderable ? { order: 'asc' } : { createdAt: 'desc' } })
...
export async function POST(req: Request, { params }: { params: { resource: string } }) {
  ...
  const last = await t.findFirst({ orderBy: { order: 'desc' }, select: { order: true } })
  const row = await t.create({ data: { ...parsed.data, order: (last?.order ?? -1) + 1 } })
